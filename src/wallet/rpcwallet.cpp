...
UniValue updatepsbtwithwallet(const JSONRPCRequest& request)
{
    CWallet * const pwallet = GetWalletForJSONRPCRequest(request);
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw std::runtime_error(
            "updatepsbtwithwallet \"hexstring\" ( sighashtype psbtformat)\n"
            ...
            "\nExamples:\n"
            + HelpExampleCli("updatepsbtwithwallet", "\"myhex\"")
            + HelpExampleRpc("updatepsbtwithwallet", "\"myhex\"")
        );

    LOCK2(cs_main, pwallet->cs_wallet);
    RPCTypeCheck(request.params, {UniValue::VSTR, UniValue::VSTR, UniValue::VBOOL}, true);
...
UniValue createpsbtfromraw(const JSONRPCRequest& request)
{
    CWallet * const pwallet = GetWalletForJSONRPCRequest(request);
    if (!EnsureWalletIsAvailable(pwallet, request.fHelp)) {
        return NullUniValue;
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
                            ...
                            + HelpExampleCli("createpsbtfromraw", "\"fundedtransactionhex\"")
                            );
...
{ //  category              name                        actor (function)           argNames
    ...
    { "rawtransactions",    "updatepsbtwithwallet",     &updatepsbtwithwallet,     {"hexstring","sighashtype","psbtformat"} },
    { "rawtransactions",    "createpsbtfromraw",        &createpsbtfromraw,        {"hexstring"} },
