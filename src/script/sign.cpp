...
static bool SignN(const SigningProvider& provider, const std::vector<valtype>& multisigdata, const BaseSignatureCreator& creator, const CScript& scriptCode, std::vector<valtype>& ret, SigVersion sigversion, std::vector<CPubKey>& key_ret)
{
    int nSigned = 0;
                     ...
                     std::vector<valtype>& ret, txnouttype& whichTypeRet, SigVersion sigversion)
{
    ...
    case TX_MULTISIG:
        ...
        return (SignN(provider, vSolutions, creator, scriptPubKey, ret, sigversion, key_ret));
...
PartiallySignedTransaction::PartiallySignedTransaction(const CMutableTransaction& tx, const std::map<uint160, CScript>& redeem_scripts, const std::map<uint256, CScript>& witness_scripts, const std::vector<PartiallySignedInput>& inputs)
{
    this->tx = tx;
